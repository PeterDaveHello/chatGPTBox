...
test(
  'fetchSSE lets the parser disambiguate scalar-like oversized SSE without content type',
  async (t) => {
    t.mock.method(console, 'debug', () => {})
    const paddingLine = `ignored: ${'x'.repeat(64 * 1024 - 16)}\n`
    const chunk = encoder.encode(`"ignored: value\n${paddingLine.repeat(129)}data: ok\n\n`)
...
test(
  'fetchSSE finishes plain JSON fallback after an empty chunk before cancellation settles',
  async (t) => {
...
test('fetchSSE rejects oversized JSON instead of silently completing', async (t) => {
  const cases = [
    ...
    {
      name: 'multibyte JSON string without content type',
      createJson: () => `"${'台'.repeat(3 * 1024 * 1024)}"`,
      contentType: null,
      expectedCancellations: 0,
    },
    {
