...
for (const event of ['close', 'error']) {
  test(`Chromium close rejects an idle transport ${event} while the browser is alive`, async (t) => {
    const fixture = await chromiumFixture(t)
    const adapter = await startChromium(fixture.options)
    fixture.sockets[0].emit(event)
    const expected = event === 'close' ? /disconnected/ : /transport error/
    await assert.rejects(adapter.close(), expected)
    await assert.rejects(adapter.close(), expected)
    await fixture.options.lifecycle.callbacks[0]()
    assert.equal(fixture.sockets[0].closeCount, event === 'close' ? 0 : 1)
  })
}
