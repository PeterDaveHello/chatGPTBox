...
test('Firefox capture retains DOM when screenshot fails', async (t) => {
  const { options } = await fixture(t, ({ path }) => {
    if (path.endsWith('/screenshot')) {
      return Response.json({ value: { error: 'unknown error', message: 'Screenshot failed' } })
    }
  })
  const adapter = await startFirefox(options)
  await assert.rejects(adapter.capture('failure'), (error) => {
    assert.ok(error instanceof AggregateError)
    assert.equal(error.errors.length, 1)
    assert.match(error.errors[0].message, /Screenshot failed/)
    assert.match(String(error), /Screenshot failed/)
    return true
  })
  assert.equal(
    await readFile(join(options.artifactsDir, 'failure.html'), 'utf8'),
    '<html>popup</html>',
  )
  await adapter.close()
})

test('Firefox capture preserves both diagnostic errors', async (t) => {
  const { options } = await fixture(t, ({ path }) => {
    if (path.endsWith('/screenshot') || path.endsWith('/source')) {
      return Response.json({ value: { error: 'unknown error', message: `Failed ${path}` } })
    }
  })
  const adapter = await startFirefox(options)
  await assert.rejects(adapter.capture('failure'), (error) => {
    ...
    assert.match(error.errors[0].message, /screenshot/)
    assert.match(error.errors[1].message, /source/)
    assert.match(String(error), /Failed \/session\/owned-session\/screenshot/)
    assert.match(String(error), /Failed \/session\/owned-session\/source/)
    return true
  })
  await adapter.close()
})

test('Firefox capture cancels both pending diagnostics with the run signal', async (t) => {
  const { options, controller, calls } = await fixture(t, ({ path, init }) => {
    if (path.endsWith('/screenshot') || path.endsWith('/source')) {
  ...
  const adapter = await startFirefox(options)
  const reason = new Error('Run cancelled during capture')
  const captured = adapter.capture('failure')
  const rejected = assert.rejects(captured, (error) => {
    assert.ok(error instanceof AggregateError)
    assert.deepEqual(error.errors, [reason, reason])
    return true
  })
  const diagnostics = calls.filter(
    ({ path }) => path.endsWith('/screenshot') || path.endsWith('/source'),
  )
  assert.equal(diagnostics.length, 2)
  controller.abort(reason)
  await rejected
  assert.ok(diagnostics.every(({ init }) => init.signal.aborted))
  await adapter.close()
})
